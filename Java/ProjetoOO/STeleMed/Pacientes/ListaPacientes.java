package ProjetoOO.STeleMed.Pacientes;

import java.util.Scanner;

import ProjetoOO.STeleMed.*;

public class ListaPacientes implements ColecaoDeUsuarios {

    Scanner leitor = new Scanner(System.in);

    private Elemento inicial;
    private int totalDePacientes;

    public ListaPacientes() {
        this.inicial = null;
        this.totalDePacientes = 0;
    }

    @Override
    public void gerenciarUsuario() {
        int opcao = -1;
        
        do {
            System.out.println("Gerenciar Paciente\n");
    
            System.out.println("Opções: ");
            System.out.println("1 - Cadastrar Paciente");
            System.out.println("2 - Atualizar Dados");
            System.out.println("3 - Remover Paciente");
            System.out.println("4 - Exibir Pacientes do Sistema");
            System.out.println("0 - Finalizar Gerenciamento\n");

            System.out.print("Digite a opção desejada: ");
            opcao = leitor.nextInt();

            leitor.nextLine(); //limpar o buffer
    
            switch (opcao) {
                case 1:
                    cadastrarUsuario();
                    break;
                
                case 2:
                    editarUsuario();
                    break;
    
                case 3:
                    excluirUsuario();
                    break;

                case 0:
                    System.out.println("\n\nSalvando dados...\n");
                    System.out.println("Dados salvos!\n\n");
                    break;
    
                default:
                    System.out.println("Opção Inválida\n\n");
                    break;
            }
        } while(opcao != 0);
    }

    @Override
    public void cadastrarUsuario() {
        System.out.println("\n\nCadastro de Paciente\n");
        
        System.out.print("Insira o nome de usuário: ");
        String nome = leitor.nextLine();
        System.out.print("Insira o cpf: ");
        String cpf = leitor.nextLine();
        System.out.print("Insira o telefone: ");
        String telefone = leitor.nextLine();
        System.out.print("Insira o endereço: ");
        String endereco = leitor.nextLine();
        System.out.print("Insira o email: ");
        String email = leitor.nextLine();
        System.out.print("Insira a senha: ");
        String senha = leitor.nextLine();
        System.out.print("Insira a data de nascimento: ");
        String dataDeNascimento = leitor.nextLine();

        Paciente paciente = new Paciente(nome, cpf, telefone, endereco, email, senha, dataDeNascimento);

        Elemento novo = new Elemento(paciente);

        if(this.inicial == null) {
            this.inicial = novo;
        } else {
            Elemento atual = this.inicial;
            while(atual.obterProximo() != null) {
                atual = atual.obterProximo();
            }
            atual.definirProximo(novo);
        }
        this.totalDePacientes++;

        System.out.println("\nCadastro realizado com êxito!\n\n");
    }
    
    @Override
    public void editarUsuario() {
        System.out.println("\n\nEditar Paciente\n");

        System.out.print("Insira o Username (email) que deseja editar: ");
        String emailBuscado = leitor.nextLine();

        Elemento atual = this.inicial;
        while(atual != null && !atual.obterPaciente().getEmail().equals(emailBuscado)) {
            atual = atual.obterProximo();
        }

        if(atual != null) {
            int opcao = -1;
            String troca = new String();
            Paciente pac = atual.obterPaciente();

            System.out.println("\nQual campo deseja modificar?\n");
            do {
                System.out.println("1 - Nome");
                System.out.println("2 - Telefone");
                System.out.println("3 - Endereço");
                System.out.println("4 - Email");
                System.out.println("5 - Senha");
                System.out.println("0 - Salvar Alterações\n");

                System.out.print("Insira a opção desejada: ");
                opcao = leitor.nextInt();

                leitor.nextLine(); //limpar o buffer

                switch (opcao) {
                    case 1:
                        System.out.print("Nome: ");
                        troca = leitor.nextLine();
                        pac.setNome(troca);
                        System.out.println("\nModificação realizada com êxito!");
                        System.out.println("\nDeseja modificar mais algum campo?\n");
                        break;

                    case 2:
                        System.out.print("Telefone: ");
                        troca = leitor.nextLine();
                        pac.setTelefone(troca);
                        System.out.println("\nModificação realizada com êxito!\n");
                        System.out.println("\nDeseja modificar mais algum campo?\n");
                        break;

                    case 3:
                        System.out.print("Endereço: ");
                        troca = leitor.nextLine();
                        pac.setEndereco(troca);
                        System.out.println("\nModificação realizada com êxito!\n");
                        System.out.println("\nDeseja modificar mais algum campo?\n");
                        break;

                    case 4:
                        System.out.print("Email: ");
                        troca = leitor.nextLine();
                        pac.setEmail(troca);
                        System.out.println("\nModificação realizada com êxito!\n");
                        System.out.println("\nDeseja modificar mais algum campo?\n");
                        break;

                    case 5:
                        System.out.print("Senha: ");
                        troca = leitor.nextLine();
                        pac.setSenha(troca);
                        System.out.println("\nModificação realizada com êxito!\n");
                        System.out.println("\nDeseja modificar mais algum campo?\n");
                        break;

                    case 0:
                        System.out.println("\nDados Salvos!\n\n");
                        break;
                
                    default:
                        System.out.println("\nOpção Inválida! Tente novamente.\n");
                        System.out.println("\nQual campo deseja modificar?\n");
                        break;
                }
            } while (opcao != 0);
        }
    }
    @Override
    public void excluirUsuario() {
        System.out.println("\n\nExcluir Paciente\n");
    
        System.out.print("\nInsira o Username (email) que deseja excluir: ");
        String emailBuscado = leitor.nextLine();
    
        Elemento atual = this.inicial;
        Elemento anterior = null;
    
        if (this.inicial != null) {
            while (atual != null && !atual.obterPaciente().getEmail().equals(emailBuscado)) {
                anterior = atual;
                atual = atual.obterProximo();
            }
    
            if (atual != null && atual.obterPaciente().getEmail().equals(emailBuscado)) {
                System.out.print("Tem certeza que deseja excluir o Paciente: " + atual.obterPaciente().getEmail() + " ? (sim/não): ");
                String confirmacao = leitor.nextLine().toLowerCase();  // converte toda a string para minúscula.
                
                if (confirmacao.equals("sim")) {
                    if (anterior == null) {
                        this.inicial = atual.obterProximo();
                    } else {
                        anterior.definirProximo(atual.obterProximo());
                    }
                    System.out.println("\nPaciente excluído com sucesso!\n\n");
                } else {
                    System.out.println("\nOperação cancelada.\n\n");
                }
            } else {
                System.out.println("\nEmail não encontrado!\n\n");
            }
        } else {
            System.out.println("\nNão existe nenhum Paciente cadastrado no sistema\n\n");
        }
    }
    
    @Override
    public String visualizarCadastros() {
        String elementos = new String();
        Elemento atual = this.inicial;

        while (atual != null) {
            // Obtendo e exibindo os dados de cada Paciente
            elementos += "Id: " + atual.obterPaciente().getId_user() + "\n"
            + "Nome: " + atual.obterPaciente().getNome() + "\n"
            + "CPF: " + atual.obterPaciente().getCpf() + "\n"
            + "Telefone: " + atual.obterPaciente().getTelefone() + "\n"
            + "Endereço: " + atual.obterPaciente().getEndereco() + "\n"
            + "Email: " + atual.obterPaciente().getEmail() + "\n"
            + "Data de Nascimento: " + atual.obterPaciente().getDataDeNascimento() + "\n"
            + "---------------------------\n";
            atual = atual.obterProximo();
        }

        return "Pacientes Cadastrados: \n\n"+ elementos + "\n";
    }
    
    public int totalDePacientes() {
        return totalDePacientes;
    }
}
